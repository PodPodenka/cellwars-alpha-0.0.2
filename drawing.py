...
class Drawing:
    ...
    def fps(self, clock):
        posx = pg.position()[0]
        posy = pg.position()[1]
        display_fps = str(int(posx))
        display_fps2 = str(int(posy))
        display_fps3 = 'X, Y Pos:'
        render = self.font.render(display_fps, 0, RED)
        self.sc.blit(render, FPS_POS)
        render2 = self.font.render(display_fps2, 0, RED)
        self.sc.blit(render2, FPS_POS2)
        render3 = self.font.render(display_fps3, 0, RED)
        self.sc.blit(render3, MTP)

    def fps2(self, clock):
        display_fps = str(int(clock.get_fps()))
        render = self.font.render(display_fps, 0, RED)
        self.sc.blit(render, FPS_POS_I)
